...
class UserController implements IController {
  ...
  private initialiseRoutes(): void {
    ...
    this.router.post(
      "/user/profile/kyc/v2",
      authenticatedMiddleware,
      this.uploadVerificationDocumentInfoV2
    );
  ...
  private addCustomCategory = async (
    req: Request | any,
    res: Response,
    next: NextFunction
  ): Promise<IUser | void> => {
  ...
  private retrieveCustomCategories = async (
    req: Request | any,
    res: Response,
    next: NextFunction
  ): Promise<IUser | void> => {
  ...
  private sendVerifyEmailToken = async (
    ...
    try {
      ...
      if (result.otp) {
        const emailTemplate: { html: string; text: string } =
          verifyEmailTemplate(result.firstname, result.otp);
        const mailService = MailService.getInstance();
        mailService.sendMail({
          to: req.email,
  ...
  private uploadVerificationDocumentInfoV2 = async (
    req: Request | any,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
    try {
      ...
      form.parse(req, async (err, fields, files) => {
        ...
        // get image file (object)
        const { frontPicture, backPicture, selfiePhoto }: any = files;
        const {
        ...
        const selfiePhotoResponse: any = await cloudinaryUpload(
          selfiePhoto.filepath
        );

        if (
          !frontPictureResponse ||
          !backPictureResponse ||
          !selfiePhotoResponse
        ) {
          return next(new HttpExeception(400, "Unable to upload photo."));
        ...
        //store uploaded image info
        const updatedUser = await this.UserService.uploadVerificationDocumentV2(
          req.user,
          identificationNumber,
          dateOfBirth,
          country,
          ...
          frontPictureResponse.secure_url,
          backPictureResponse.secure_url,
          selfiePhotoResponse.secure_url
        );
