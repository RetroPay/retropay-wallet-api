...
class BudgetService {
    ...
    public async createNairaMonthlyBudget(userId: string, referenceId: string, k_token: string, totalBudgetAmount: number, budgetMonth: string, budgetYear: string, budgetName: string, budgetItems: {}[]): Promise<IBudget> {
        try {
             ...
             * get user info
             * get user balance
             * check if user balance is less than budget total amount (for monthly budget - totalBudgetAmount is calculated from all items)
             * attempt to create budget on kuda
             * Save budget to DB plus saving ID
             * Add the money into the savings
             */
    ...
    public async fundNairaBudget(k_token: string, amount: number, budgetName: string, budgetUniqueId: string): Promise<any> {
      try {
        const response = await axios({
          method: "POST",
          url:
