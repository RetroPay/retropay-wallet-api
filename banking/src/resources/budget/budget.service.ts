...
class BudgetService {
  ...
  public async createBudget(userId: string, referenceId: string, k_token: string, totalBudgetAmount: number, startDate: string, endDate: string, budgetName: string, budgetItems: {}[], currency: string): Promise<IBudget> {
    try {
      switch (currency.toLocaleLowerCase()) {
        case 'ngn': {
          ...
          break;
        default: throw new Error(`${currency} budgets are currently not supported.`)
          break;
      }
    } catch (error) {
  ...
  private async createNairaBudgetAccount(userId: string, referenceId: string, k_token: string, totalBudgetAmount: number, startDate: string, endDate: string, budgetName: string, budgetItems: {}[], currency: string) {
    try {
      ...
      if (!fundBudgetResponseData.status) throw new Error("We were unable to fund this budget, please try again");
  ...
  private async topUpNairaBudgetItem(k_token: string, amount: number, budgetUniqueId: string, budgetItemId: string): Promise<any> {
    try {
      const response = await axios({
        method: "POST",
        url:
